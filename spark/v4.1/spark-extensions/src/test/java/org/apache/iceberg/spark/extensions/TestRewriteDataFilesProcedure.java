...
public class TestRewriteDataFilesProcedure extends ExtensionsTestBase {
  ...
  //        .isEqualTo("c1");
  //    assertThat(order.get(1).term()).as("Second field should be
  // zorder").isInstanceOf(Zorder.class);
  //  }

  //  @TestTemplate
  //  public void testRewriteDataFilesInEmptyTable() {
  //    createTable();
  //    List<Object[]> output = sql("CALL %s.system.rewrite_data_files('%s')", catalogName,
  // tableIdent);
  ...
  public void testRewriteDataFilesOnNonPartitionTable() {
    ...
    System.out.println("fffffffffffffffffFiles created: " + files.get(0)[0]);
    List<Object[]> expectedRecords = currentData();
  ...
  //    // As there is only one small output file, we can validate the query ordering (as it will
  // not
  //    // change).
  //    ImmutableList<Object[]> expectedRows =
  //        ImmutableList.of(
  ...
  //    // currently spark session catalog only resolve to v1 functions instead of desired v2
  // functions
  //    //
  ...
  //    // And
  //    sql(
  //        "CALL %s.system.rewrite_data_files(table => '%s'," + " where => 'c1 = 3 and c2 =
  // \"bar\"')",
  //        catalogName, tableIdent);
  //    // Or
  //    sql(
  ...
  //            () ->
  //                sql(
  //                    "CALL %s.system.rewrite_data_files(table => '%s', " + "where => 'col1 =
  // 3')",
  //                    catalogName, tableIdent))
  //        .isInstanceOf(IllegalArgumentException.class)
  ...
  //                + "struct<1: c1: optional int, 2: c2: optional string, 3: c3: optional
  // string>");
  //
  ...
  //        .hasMessage(
  //            "[DUPLICATE_ROUTINE_PARAMETER_ASSIGNMENT.BOTH_POSITIONAL_AND_NAMED] Call to routine
  // `rewrite_data_files` is invalid because it includes multiple argument assignments to the same
  // parameter name `table`. A positional argument and named argument both referred to the same
  // parameter. Please remove the named argument referring to this parameter. SQLSTATE: 4274K");
  //
  ...
  //        .isInstanceOf(AnalysisException.class)
  //        .hasMessage(
  //            "[REQUIRED_PARAMETER_NOT_FOUND] Cannot invoke routine `rewrite_data_files` because
  // the parameter named `table` is required, but the routine call did not supply a value. Please
  // update the routine call to supply an argument value (either positionally at index 0 or by name)
  // and retry the query again. SQLSTATE: 4274K");
  //
  //    assertThatThrownBy(
  //            () -> sql("CALL %s.system.rewrite_data_files(table => 't', table => 't')",
  // catalogName))
  //        .isInstanceOf(AnalysisException.class)
  //        .hasMessageEndingWith(
  //            "[DUPLICATE_ROUTINE_PARAMETER_ASSIGNMENT.DOUBLE_NAMED_ARGUMENT_REFERENCE] Call to
  // routine `rewrite_data_files` is invalid because it includes multiple argument assignments to
  // the same parameter name `table`. More than one named argument referred to the same parameter.
  // Please assign a value only once. SQLSTATE: 4274K");
  //
  //    assertThatThrownBy(() -> sql("CALL %s.system.rewrite_data_files('')", catalogName))
  //        .isInstanceOf(IllegalArgumentException.class)
  ...
  //            () ->
  //                sql(
  //                    "CALL %s.system.rewrite_data_files(table => '%s', where => 'substr(c2, 2) =
  // \"fo\"')",
  //                    catalogName, tableIdent))
  //        .isInstanceOf(IllegalArgumentException.class)
  ...
  //                .sql(
  //                    String.format(
  //                        "SELECT * FROM %s.branch_%s ORDER BY c1, c2, c3", tableName,
  // branchName))
  //                .collectAsList());
  //
  ...
  //    List<Object[]> output =
  //        sql(
  //            "CALL %s.system.rewrite_data_files(table => '%s', branch => '%s', where => 'c2 =
  // \"bar\"')",
  //            catalogName, tableName, branchName);
  //
  //    assertEquals(
